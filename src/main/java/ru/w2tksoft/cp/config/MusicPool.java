...
import ru.w2tksoft.cp.control.Audio;
import ru.w2tksoft.cp.control.ErrorCommonManager;
import ru.w2tksoft.cp.control.Sound;
import ru.w2tksoft.cp.control.SoundSource;

import java.io.IOException;

//Объект контролирует загрузку, запуск и удаление всей музыки на протяжении всей игры
...
public class MusicPool {
    ...
    public MusicPool(Audio audio) {
        this.audio = audio;
        if (audio != null)
            music = new Entry[35];
    }

    /*private void changeMusic(String newMusic, ErrorCommonManager errorCommonManager) {
        if (audio == null)
            return;
        try {
            int index = musicArrayNum(newMusic);
            if (music[index] == null) {
                music[index] = new Entry();
                music[index].snd = new Sound()
            }
        }
    }
